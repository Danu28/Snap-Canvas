...
- [x] F1. Editor "Clear All" button (user request: add a small useful feature) — done 2026-08-12  (verify: new Clear All button in .actions clears all annotations in one click via clearAll() — reuse of resetDragState/commitHistory/redraw, push to history so Undo restores; no-op + status when empty; gate PASS (node --check all); H1 golden compare PASS 11/11 export steps bit-identical, 0 page/runtime errors — the harness session doesn't touch the new button)  files: editor.html, editor.js
